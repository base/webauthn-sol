...
contract VerifySimTest is Test {
    ...
    function test_verifySim_returnsTrue_withInvalidSignature() public {
        // Start from a known-good WebAuthnAuth and public key, then corrupt r to make it invalid.
        ...
        // Deliberately break the signature to simulate invalid caller inputs.
        auth.r ^= 1;

        // First, the real verifier should fail with the same invalid inputs.
        bool realResult = WebAuthn.verify(challenge, false, auth, x, y);
        assertFalse(realResult);

        // Then, verifySim should succeed with the same inputs.
        bool simResult = WebAuthn.verifySim(challenge, false, auth, x, y);
